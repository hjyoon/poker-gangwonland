...
싱글플레이와 멀티플레이에서는 앱 진행용 멀티 테이블 프리즈아웃 토너먼트를 만들 수 있습니다. 최초 참가자 수에는 인간과 컴퓨터가 모두 포함되며, 서버가 참가자를 테이블당 최대 8명으로 무작위 균등 배치합니다. 싱글플레이는 인간 참가자 1명과 나머지 컴퓨터 참가자로 구성해 공유 대기실 없이 즉시 시작하고, 화면에 표시되지 않은 테이블도 서버에서 진행합니다. 연결이 끊어진 인간 참가자는 탈락하지 않고 블라인드를 계속 납부하며 재접속 전까지 자동 폴드됩니다. 이 토너먼트 구조는 강원랜드 공식 운영 규정이 아니라 현재 앱 규칙을 이용한 진행 기능입니다.
...
- 쇼다운 후 수수료 5% 차감 정산
- 싱글플레이와 멀티플레이에서 인간과 컴퓨터를 합쳐 최대 64명으로 시작하는 멀티 테이블 프리즈아웃 토너먼트
