...
싱글플레이와 멀티플레이에서는 앱 진행용 멀티 테이블 프리즈아웃 토너먼트를 만들 수 있습니다. 최초 참가자 수에는 인간과 컴퓨터가 모두 포함되며 최대 128명까지 설정할 수 있고, 서버가 참가자를 테이블당 최대 8명으로 무작위 균등 배치합니다. 하나의 블라인드 레벨은 동기화된 10라운드 동안 유지되며, 다음 레벨에서는 SB/BB뿐 아니라 모든 스트리트의 베팅·레이즈 단위와 상한, 한 핸드 1인 최대 총베팅이 직전 레벨의 정확히 2배로 상승합니다. 싱글플레이는 인간 참가자 1명과 나머지 컴퓨터 참가자로 구성해 공유 대기실 없이 즉시 시작하고, 화면에 표시되지 않은 테이블도 서버에서 진행합니다. 다음 라운드 자동 진행은 기본적으로 꺼져 있어 모든 테이블이 끝난 뒤에도 카드와 승부 결과를 원하는 만큼 확인하고 직접 다음 라운드를 시작할 수 있으며, 설정에서 자동 진행과 딜레이를 다시 활성화할 수 있습니다. 참가자는 다른 테이블의 공개 상황을 실시간으로 관전할 수 있으며 비공개 개인 카드는 노출되지 않고, 자신의 차례가 오면 실제 참가 테이블로 자동 복귀합니다. 연결이 끊어진 인간 참가자는 탈락하지 않고 블라인드를 계속 납부하며 재접속 전까지 자동 폴드됩니다. 이 토너먼트 구조와 블라인드·베팅 레벨은 강원랜드 공식 운영 규정이 아니라 현재 앱 규칙을 이용한 진행 기능입니다.
...
- 쇼다운 후 수수료 5% 차감 정산
- 싱글플레이와 멀티플레이에서 인간과 컴퓨터를 합쳐 최대 128명으로 시작하는 멀티 테이블 프리즈아웃 토너먼트
- 인간 1명과 나머지 컴퓨터로 즉시 시작하고, 인간 행동 제한 시간을 두지 않는 싱글플레이 토너먼트
- 테이블당 최대 8명 무작위 균등 배치와 핸드 경계 재배치
- 다른 테이블 실시간 관전, 비공개 카드 보호, 자기 차례 자동 복귀
