...
싱글플레이와 멀티플레이에서는 앱 진행용 멀티 테이블 프리즈아웃 토너먼트를 만들 수 있습니다. 최초 참가자 수에는 인간과 컴퓨터가 모두 포함되며 최대 128명까지 설정할 수 있고, 서버가 참가자를 테이블당 최대 8명으로 무작위 균등 배치합니다. 각 라운드는 같은 번호의 블라인드 레벨을 사용하며 SB는 2,000원, BB는 5,000원씩 상승해 레벨 20의 SB 40,000원 / BB 100,000원에서 고정됩니다. 싱글플레이는 인간 참가자 1명과 나머지 컴퓨터 참가자로 구성해 공유 대기실 없이 즉시 시작하고, 화면에 표시되지 않은 테이블도 서버에서 진행합니다. 참가자는 다른 테이블의 공개 상황을 실시간으로 관전할 수 있으며 비공개 개인 카드는 노출되지 않고, 자신의 차례가 오면 실제 참가 테이블로 자동 복귀합니다. 연결이 끊어진 인간 참가자는 탈락하지 않고 블라인드를 계속 납부하며 재접속 전까지 자동 폴드됩니다. 이 토너먼트 구조와 블라인드 레벨은 강원랜드 공식 운영 규정이 아니라 현재 앱 규칙을 이용한 진행 기능입니다.
