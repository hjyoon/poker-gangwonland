...
export function startNewHand({ cpuCount, dealerIndex, chipTotals, feeTotal = 0, handNumber = 1, computerStyle = "balanced" }) {
  ...
  if (playableCount < 2) {
    return {
      ...
      currentBet: 0,
      currentHandFee: 0,
      computerStyle: getComputerStyle(computerStyle).key,
      handNumber,
...
export function chooseComputerAction(state, actorIndex = state.currentPlayerIndex) {
  ...
  const style = getComputerStyle(state.computerStyle);
