...
function finalizeShowdown(state) {
  ...
  const finalizedShowdownResults = openedContenders.map((player) => resultMap.get(player.id) ?? {
    id: player.id,
    name: player.name,
...
function adaptiveShowdownAdjustment(style, state, player, level) {
  ...
  const foldRate = activeOpponentStats.reduce((total, stats) => total + stats.folds, 0) / totalActions;
  const aggressionRate = activeOpponentStats.reduce((total, stats) => total + stats.aggressiveActions, 0) / totalActions;
  let threshold = 0;
...
function chooseComputerShowdownAction(state, actorIndex, actions) {
  ...
  const adaptiveAdjustment = adaptiveShowdownAdjustment(style, state, player, level);
  const losingShowChance = Math.min(
    0.35,
    profile.losingShowChance + closeHandRevealChance + adaptiveAdjustment.losingShowChance + level.mistakeRate * 0.5,
  );
