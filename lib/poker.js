...
function buildComputerLevelMap(cpuCount, computerLevels = {}, fallbackLevel = "intermediate") {
  return buildComputerLevelMapForPlayers(createPlayers(cpuCount, false), computerLevels, fallbackLevel);
}

function buildComputerLevelMapForPlayers(players, computerLevels = {}, fallbackLevel = "intermediate") {
...
export function startNewHand({
  ...
  players[smallBlindIndex].actionLocked = players[smallBlindIndex].chipBalance === 0;
  ...
  players[bigBlindIndex].actionLocked = players[bigBlindIndex].chipBalance === 0;
...
export function getAvailableActions(state, playerIndex) {
  ...
  return [
    ...
    isOpeningBet
      ? {
          key: "bet",
          label: canWager && isAllIn ? `베팅 (Bet) 올인 ${formatMoney(nextWagerTarget)}` : `베팅 (Bet) ${formatMoney(nextWagerTarget)}`,
          enabled: canWager,
        }
      : {
          key: "raise",
          label: canWager && isAllIn ? `레이즈 (Raise) 올인 ${formatMoney(nextWagerTarget)}` : `레이즈 (Raise) ${formatMoney(nextWagerTarget)}`,
          enabled: canWager,
        },
  ];
}
...
export function applyAction(state, actionKey, actorIndex = state.currentPlayerIndex) {
  ...
  if (actionKey === "bet" || actionKey === "raise") {
    ...
    if (
      ...
      log.push(`${actor.name}: ${actionLabel} ${formatMoney(target)}${actor.actionLocked ? " (올인)" : ""}`);
...
function computerCallAdjustment(player, state, style, level, toCall) {
  ...
  if (lastAggressor?.isHuman) {
    opponentAdjustment += style.bluffCatchBonus;
  }
...
export function chooseComputerAction(state, actorIndex = state.currentPlayerIndex) {
  ...
  const adjustedCallStrength =
    judged + computerCallAdjustment(player, state, style, level, toCall) + positionContext.callAdjustment * level.positionWeight + drawAdjustment;
  ...
  const wagerThreshold = style.wagerAbove - positionContext.wagerAdjustment * level.positionWeight - drawAdjustment * 0.5;
