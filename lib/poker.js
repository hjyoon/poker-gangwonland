...
export function chooseComputerAction(state, actorIndex = state.currentPlayerIndex) {
  ...
  const adjustedCallStrength = strength + computerCallAdjustment(player, state, style, toCall) + positionContext.callAdjustment;
  const openingAction = actions.find((action) => action.key === "bet");
  const adjustedOpeningStrength = strength + computerOpeningAdjustment(player, state, style) + positionContext.openingAdjustment;
  const wagerThreshold = style.wagerAbove - positionContext.wagerAdjustment;

  if (strength < style.hardFoldBelow && toCall > 0) {
    return "fold";
  }
  const aggressiveAction = actions.find((action) => action.key === "raise" || action.key === "bet");
  if (strength > wagerThreshold && aggressiveAction) {
