...
const HAND_LABELS = [
  "제공된 기준에는 해당 패 명칭이 명시되어 있지 않습니다",
  "원 페어",
  "투 페어",
  "쓰리 오브 카인드",
  "스트레이트",
...
export function startNewHand({ cpuCount, dealerIndex, chipTotals }) {
  ...
  return {
    ...
    chipTotals,
    note:
      "카드 서열 표시는 AGENTS.md 기준을 우선 따릅니다. 제공된 카드 서열에 없는 패 명칭은 별도 운영 규정 확인이 필요합니다.",
  };
}
...
export function applyAction(state, actionKey, actorIndex = state.currentPlayerIndex) {
  ...
  if (actionKey === "raise") {
    ...
    if (target <= street.maxBet && actor.totalContribution + amount <= 100000) {
