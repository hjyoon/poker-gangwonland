...
export function createPlayers(cpuCount, includeHuman = true) {
...
function buildComputerStyleMap(cpuCount, computerStyles = {}, fallbackStyle = "balanced") {
  ...
  return Object.fromEntries(
    Array.from({ length: cpuCount }, (_, index) => {
      const playerId = `cpu-${index + 1}`;
      return [playerId, getComputerStyle(styleMap[playerId] ?? fallback).key];
    }),
  );
}
...
export function startNewHand({
  ...
  const resolvedComputerStyles = buildComputerStyleMap(cpuCount, computerStyles, computerStyle);
  const players = createPlayers(cpuCount, includeHuman).map((player) => {
...
function finalizeShowdown(state) {
  ...
  const payouts = splitPotWithFees(state.pot, winners);
  const fee = payouts.reduce((total, payout) => total + payout.fee, 0);
  const feeTotal = (state.feeTotal ?? 0) + fee;
  const playersAfterPayout = clonePlayers(state.players).map((player) => {
    const payout = payouts.find((entry) => entry.id === player.id);
    if (!payout) {
      return player;
    }
    return {
  ...
  const resultLog =
    winners.length === 1
      ? `${winners[0].name} 승리. 수수료 5% 제외 후 ${formatMoney(payouts[0].net)} 획득.`
      : `동률 발생: ${payouts
          .map((payout) => {
            const winner = winners.find((entry) => entry.id === payout.id);
...
export function chooseComputerAction(state, actorIndex = state.currentPlayerIndex) {
  ...
  const style = getComputerStyle(player.computerStyle ?? state.computerStyles?.[player.id] ?? state.computerStyle);
