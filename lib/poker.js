...
export function startNewHand({
  ...
  const resolvedBigBlindAmount = Math.max(
    ...
    Math.min(MAX_PLAYER_TOTAL_BET, Math.max(MIN_PLAYABLE_BALANCE, Math.trunc(Number(bigBlindAmount)))),
  );
  ...
  const { total: forcedContributionTotal, log: forcedContributionLog } = applyForcedContributions(players, forcedContributions);
...
function remainingContributionCap(player) {
  return Math.max(0, MAX_PLAYER_TOTAL_BET - player.totalContribution);
}
...
function applyForcedContributions(players, forcedContributions = []) {
  const log = [];
  const total = forcedContributions.reduce((sum, contribution) => {
    ...
    const amount = Math.min(requestedAmount, Math.max(0, player.chipBalance), remainingContributionCap(player));
    if (amount <= 0) {
      return sum;
    }
...
export function getAvailableActions(state, playerIndex) {
  ...
  const canWager =
    ...
    player.totalContribution + wagerAmount <= MAX_PLAYER_TOTAL_BET;
...
export function applyAction(state, actionKey, actorIndex = state.currentPlayerIndex) {
  ...
  if (actionKey === "bet" || actionKey === "raise") {
    ...
    if (
      ...
      actor.totalContribution + amount <= MAX_PLAYER_TOTAL_BET
    ) {
